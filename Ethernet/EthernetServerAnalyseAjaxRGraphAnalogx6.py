...
<html>
    <head>
	   ...
	   // code javascript par X. HINAULT - www.mon-club-elec.fr - tous droits reserves - 2013 - GPL v3
        ...
        window.onload = function () { // au chargement de la page
			...
			gauge[1] = new RGraph.Gauge('CanvasRGraph1', 0, 2000, 0); // declare widget graphique
			gauge[1].Draw(); // dessine le widget graphique dans le canvas

		   gauge[2] = new RGraph.Gauge('CanvasRGraph2', 0, 2000, 0); // declare widget graphique
			gauge[2].Draw(); // dessine le widget graphique dans le canvas

		   gauge[3] = new RGraph.Gauge('CanvasRGraph3', 0, 3300, 0); // declare widget graphique
			gauge[3].Draw(); // dessine le widget graphique dans le canvas

		   gauge[4] = new RGraph.Gauge('CanvasRGraph4', 0, 3300, 0); // declare widget graphique
			gauge[4].Draw(); // dessine le widget graphique dans le canvas

		   gauge[5] = new RGraph.Gauge('CanvasRGraph5', 0, 3300, 0); // declare widget graphique
			gauge[5].Draw(); // dessine le widget graphique dans le canvas

		   gauge[6] = new RGraph.Gauge('CanvasRGraph6', 0, 3300, 0); // declare widget graphique
    ...
    <body>  
                       ...
                       <table>
                        ...
                        <canvas id="CanvasRGraph1" width="250" height="250">[Canvas non disponible]</canvas>
                         <br />
                         <div style="text-align: center">Valeur 1 (0-2000 mV) = </div> <input type="text" id="valeurRGraph1" />               
                                        </td>

                                        <td>
                                        <canvas id="CanvasRGraph2" width="250" height="250">[Canvas non disponible]</canvas>
                         <br />
                         <div style="text-align: center">Valeur 2 (0-2000 mV) = </div> <input type="text" id="valeurRGraph2" />               
                                        </td>

                                        <td>
                                        <canvas id="CanvasRGraph3" width="250" height="250">[Canvas non disponible]</canvas>
                         <br />
                          <div style="text-align: center">Valeur 3 (0-3300 mV) = </div> <input type="text" id="valeurRGraph3" />               
                                       </td>
                         ...
                         <br />
                          <div style="text-align: center">Valeur 4 (0-3300 mV) = </div> <input type="text" id="valeurRGraph4" />               
                                       </td>

                                        <td>
                                        <canvas id="CanvasRGraph5" width="250" height="250">[Canvas non disponible]</canvas>
                         <br />
                         <div style="text-align: center">Valeur 5 (0-3300 mV) = </div> <input type="text" id="valeurRGraph5" />               
                                        </td>

                                        <td>
                                        <canvas id="CanvasRGraph6" width="250" height="250">[Canvas non disponible]</canvas>
                         <br />
                         <div style="text-align: center">Valeur 6 (0-3300 mV) = </div> <input type="text" id="valeurRGraph6" />               
                                        </td>
	...
	<br/>
	Serveur Pyduino : Affichage analogique RGraph x6 avec reponse de requete Ajax 
	<br/>
...
def reponseAJAXServeur():
	...
	values=list() # declare la liste de stockage des valeurs analogiques
	
	for i in range(6): # defile les 6 voies
		values.append(analogReadmV(i)) # mesure mV sur la voie i
	
	# la reponse 
	reponseAjax=( # debut page reponse AJAX
#"""123, 456, 789""" 
""+str(values[A0]) +","+str(values[A1]) +","+str(values[A2])  +","+str(values[A3])  +","+str(values[A4])  +","+str(values[A5])
