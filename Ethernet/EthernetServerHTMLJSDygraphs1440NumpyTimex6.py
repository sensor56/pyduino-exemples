...
#""")
	...
	data=np.zeros((1440,nombreSeries+1)) # tableau de 1000 lignes x 2 colonnes
	...
	for serie in range(1,nombreSeries+1): # defile serie des y de 1 à n
		data[:,serie] = (serie*5)+np.random.normal(0,1,size=1440)  # remplissage 2eme colonne = les y1 
	...
	for t in range(1440-1) : # defile 1440-1 ères minutes theoriques
		...
		dataLine=str(dataTime)+","# format datetime JJ-MM-AAAA hh:mm:ss
		
		for serie in range(1,nombreSeries+1): # defile serie des y de 1 à n
			if serie==nombreSeries: 
				dataLine=dataLine+str(data[t,serie]) # sans la virgule
			else : 
				dataLine=dataLine+str(data[t,serie])+"," # sinon avec la virgule

		dataGraph=dataGraph+"\""+dataLine+"\\n \" +"+"\n" # format texte dans code JS 
		# pas le 2eme \n si on veut source page compact... = "illisible" cote client
	
	# derniere ligne 
	dataTime=refTime+datetime.timedelta(0, 1440*60) # jours, secondes - ici toutes les minutes
	dataLine=str(dataTime)+","# format datetime JJ-MM-AAAA hh:mm:ss
	
	for serie in range(1,nombreSeries+1): # defile serie des y de 1 à n
		if serie==nombreSeries: 
			dataLine=dataLine+str(data[-1,serie]) # sans la virgule
		else : 
			dataLine=dataLine+str(data[-1,serie])+"," # sinon avec la virgule
