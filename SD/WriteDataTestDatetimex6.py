...
# Creer un fichier de 6 series de donnees horodatees de test sur 24H
...
def setup():
	...
	nombreSeries=6 # nombre serie de donnees
	...
	for t in range(1440) : # defile 1440 minutes theoriques
		...
		# format de donnees utilise : JJ/MM/YYYY hh:mm:ss , val1, val2, .. , val6 \n
		
		dataLine=str(dataTime)+","# format datetime JJ-MM-AAAA hh:mm:ss
		
		for serie in range(1,nombreSeries+1): # defile serie des y de 1 à n
			
			dataValue=str(((serie-1)*1023)+random(0,1023)) # genere une valeur aleatoire entiere
			
			if serie==nombreSeries: 
				dataLine=dataLine+str(dataValue) + "\n"# sans la virgule
			else : 
				dataLine=dataLine+str(dataValue)+"," # sinon avec la virgule
